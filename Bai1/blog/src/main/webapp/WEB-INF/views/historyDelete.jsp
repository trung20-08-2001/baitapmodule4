...
<div>
    ...
    <div style="float: left;width: 57%;position: absolute; left:31%">
        <h1 style="text-align: center;color: red">Deleted post</h1>
        <!-- -----------------------------------
        ...
        <div>
            <div class="w3-container w3-white w3-padding-large">
                <c:if test="${historyDeletedList.size()==0}">
                    <p class="text-success" style="text-align: center; font-size: 50px">There are no posts</p>
                </c:if>
                <c:if test="${historyDeletedList.size()!=0}">
                    <c:forEach var="post" items="${historyDeletedList}">
                        <div class="w3-center">
                            <h3>${post.title}</h3>
                            <h5>Creation Time <span class="w3-opacity">${post.creationTimeString()}</span></h5>
                            <h5>Creation Time <span class="w3-opacity">${post.deleteTimeString()}</span></h5>
                        </div>

                        <div class="w3-justify">
                            <a href="/detail?idPost=${post.idPost}"><img src="${post.image}" alt="Image "
                                                                         style="width:100%;height:350px"
                                                                         class="w3-padding-16"></a>
                            <p class="truncated-text">${post.content}</p>
                            <span><h5><a href="/detail?idPost=${post.idPost}">See more</a></h5></span>
                            <p class="w3-clear"></p>
                        ...
                        <a href="/restore?idPost=${post.idPost}" type="button" class="btn btn-success">Restore</a>
                        <br>
                        <hr>
                        <br>
                    </c:forEach>
